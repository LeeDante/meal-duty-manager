...
async function importLegacy(env,payload,actor){
  ...
  try{
    ...
    stage='店家';await runBatch(env,stores.map(s=>env.DB.prepare("INSERT INTO stores(id,name,sort_order,active) VALUES(?,?,?,?) ON CONFLICT(id) DO UPDATE SET name=excluded.name,sort_order=excluded.sort_order,active=excluded.active").bind(s.id,s.name,s.sort||0,s.active===false?0:1)));
    stage='店家菜單';await runBatch(env,(src.menu||[]).map(x=>env.DB.prepare("INSERT INTO menu_items(id,store_id,name,price,sort_order,active) VALUES(?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET store_id=excluded.store_id,name=excluded.name,price=excluded.price,sort_order=excluded.sort_order,active=excluded.active").bind(x.id,x.storeId,x.name,x.price||0,x.sort||0,x.active===false?0:1)));
    ...
    stage='餐次菜單';await runBatch(env,(src.mealMenus||[]).map(x=>env.DB.prepare("INSERT INTO meal_menu_items(id,meal_id,source_id,store_id,name,price,special,sort_order,active) VALUES(?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET meal_id=excluded.meal_id,source_id=excluded.source_id,store_id=excluded.store_id,name=excluded.name,price=excluded.price,special=excluded.special,sort_order=excluded.sort_order,active=excluded.active").bind(x.id,x.mealId,x.sourceId||null,x.storeId||null,x.name,x.price||0,x.special?1:0,x.sort||0,x.active===false?0:1)));
    stage='訂單';await runBatch(env,(src.orders||[]).map(o=>env.DB.prepare("INSERT INTO orders(id,meal_id,event_id,person_id,meal_menu_id,name,price,qty,note,active,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET meal_id=excluded.meal_id,event_id=excluded.event_id,person_id=excluded.person_id,meal_menu_id=excluded.meal_menu_id,name=excluded.name,price=excluded.price,qty=excluded.qty,note=excluded.note,active=excluded.active,updated_at=excluded.updated_at").bind(o.id,o.mealId,o.eventId,o.personId,o.mealMenuId,o.name,o.price||0,o.qty||1,o.note||'',o.active===false?0:1,o.updatedAt||now())));
...
export async function onRequest({request,env,params}){
  ...
  try{
    ...
    if(path==='/admin/bootstrap'&&method==='GET'){const s=await session(env,request,'admin');if(!s)return json({ok:false,message:'管理權限已失效'},401);const counts={};for(const t of ['people','stores','events','meals','orders','logs'])counts[t]=(await env.DB.prepare(`SELECT COUNT(*) n FROM ${t}`).first()).n;const [people,stores,menu]=await Promise.all([env.DB.prepare("SELECT id,name,group_name AS 'group',sort_order AS sortOrder,CASE WHEN employee_code_hash IS NULL THEN 0 ELSE 1 END AS hasCode FROM people WHERE active=1 ORDER BY sort_order,name").all(),env.DB.prepare("SELECT id,name,sort_order AS sortOrder FROM stores WHERE active=1 ORDER BY sort_order,name").all(),env.DB.prepare("SELECT id,store_id AS storeId,name,price,sort_order AS sortOrder FROM menu_items WHERE active=1 ORDER BY store_id,sort_order,name").all()]);return json({ok:true,counts,people:people.results,stores:stores.results,menu:menu.results});}
    ...
    if(path==='/admin/person-active'&&method==='POST'){const s=await session(env,request,'admin');if(!s)return json({ok:false,message:'管理權限已失效'},401);const d=await body(request);await env.DB.prepare('UPDATE people SET active=? WHERE id=?').bind(d.active?1:0,d.id).run();await log(env,'管理者',d.active?'RESTORE_PERSON':'DISABLE_PERSON','person',d.id,'');return json({ok:true});}
    if(path==='/admin/stores'&&method==='POST'){const s=await session(env,request,'admin');if(!s)return json({ok:false,message:'管理權限已失效'},401);const d=await body(request),name=String(d.name||'').trim();if(!name)return json({ok:false,message:'請輸入店家名稱'},400);if(d.id){await env.DB.prepare('UPDATE stores SET name=? WHERE id=?').bind(name,d.id).run();await log(env,'管理者','UPDATE_STORE','store',d.id,name);return json({ok:true,id:d.id})}const id=crypto.randomUUID(),sort=(await env.DB.prepare('SELECT COALESCE(MAX(sort_order),0)+1 n FROM stores').first()).n;await env.DB.prepare('INSERT INTO stores(id,name,sort_order,active) VALUES(?,?,?,1)').bind(id,name,sort).run();await log(env,'管理者','ADD_STORE','store',id,name);return json({ok:true,id});}
    ...
    if(path==='/admin/menu-items'&&method==='POST'){const s=await session(env,request,'admin');if(!s)return json({ok:false,message:'管理權限已失效'},401);const d=await body(request),name=String(d.name||'').trim(),price=Math.max(0,Math.round(Number(d.price)||0));if(!name||!d.storeId)return json({ok:false,message:'店家與餐點名稱不可空白'},400);if(d.id){await env.DB.prepare('UPDATE menu_items SET name=?,price=? WHERE id=? AND store_id=?').bind(name,price,d.id,d.storeId).run();await log(env,'管理者','UPDATE_MENU','menu',d.id,`${name}｜${price}`);return json({ok:true,id:d.id})}const id=crypto.randomUUID(),sort=(await env.DB.prepare('SELECT COALESCE(MAX(sort_order),0)+1 n FROM menu_items WHERE store_id=?').bind(d.storeId).first()).n;await env.DB.prepare('INSERT INTO menu_items(id,store_id,name,price,sort_order,active) VALUES(?,?,?,?,?,1)').bind(id,d.storeId,name,price,sort).run();await log(env,'管理者','ADD_MENU','menu',id,`${name}｜${price}`);return json({ok:true,id});}
